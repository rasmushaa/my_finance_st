...
def st_wrapper_filetype_in_database(df):
    return st.session_state.backend.filesystem.database.filetype_is_in_database(df)

@st.cache_data(show_spinner='Transforming the File...')
def st_wrapper_transform_input_file(df):
    return st.session_state.backend.filesystem.parser.transform_input_file(df)


# Dymaic Visualization Funcs
def load_file():
    if st.session_state['input_file'] is not None:
        st.session_state['banking_file'] = st.session_state.backend.filesystem.parser.open_binary_as_pandas(st.session_state['input_file'])
...
def validate_filetype() -> bool:
    if st_wrapper_filetype_in_database(st.session_state['banking_file']):
        ...
        collector = st.session_state.backend.filesystem.database.get_filetype_data_collector()
        ...
        if st.button('Add the Filetype to the Database', width='stretch'):
            if st.session_state.backend.filesystem.database.add_filetype_to_databases(collector):
